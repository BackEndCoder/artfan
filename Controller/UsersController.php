...
class UsersController extends AppController {
	...
	public function login() {
		if (isset($user['role_id'])) {
			...
			if ($this->request->is('post')) {
				...
				$userExists = $this->User->find('first', array('conditions' =>
		...
		if (isset($user['role_id'])) {
			...
			if ($this->request->is('post')) {
				...
				if (!empty($userExists)) {
					...
					$this->request->data['User']['randStr'] = $rand;
					if ($this->User->save($this->request->data)) {
						...
						$userRoleId = $this->request->data['User']['role_id'];
	...
	public function profile() {
		$usr = $this->Auth->user();
		$dbUser = $this->User->find('first', array(
			'conditions' => array(
				'User.id' => $this->Auth->user('id'))
		));
		$folderUrl = WWW_ROOT . "files/ProfileImages/" . $this->Auth->user('id') . "/";
		if ($this->request->is('post') || $this->request->is('put')) {
			if ($this->User->save($this->request->data)) {
				...
				if ($image['size'] > 0) {
					...
					move_uploaded_file($image['tmp_name'], $folderUrl . $image['name']);
				}
			...
			$httpUrl = $this->base . "/files/ProfileImages/" . $this->Auth->user('id') . "/";
			$images = '';
	...
	public function changepassword() {
		...
		if ($this->request->is('post')) {
			...
			$dbPassword = $dbUser['User']['password'];

			if (AuthComponent::password($currentPassword) != $dbPassword) {
